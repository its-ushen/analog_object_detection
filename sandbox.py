from ushen_final_helper import run_yolo, process_depth
...
def depth_detection():
    ...
    while True:
        ...
        success, depth_frame = capture.get_transformed_depth_image()
        if not ret or not success:  # break if no frame
            print("Issue with capturing depth frame")
            break

        # YOLO detection
        frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
        frame, box_centers, locations_list = run_yolo(frame)

        frame, depth_distance = process_depth(
            box_centers, frame, depth_frame, locations_list)
