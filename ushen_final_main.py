...
def depth_detection():
    ...
    while True:
        ...
        if not ret or not success:  # break if no frame
            print("issue with capturing depth frame I think")
            break


        # YOLO detection
        frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
        frame, box_centre, locations = run_yolo(frame)
        # print("Ran YOLO")

        frame, depth_distance = process_depth(box_centre, frame, depth_frame, locations)
